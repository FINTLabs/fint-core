...
Which relations get back-links from autorelation is listed in
[RELATION_RULES.md](RELATION_RULES.md). That file is generated by a test in
`fint-core-shared`, so it cannot fall behind the code: the test fails and rewrites the
file when the rules or the information-model version change, and you commit the result.
