...
class ResponseEventTopicEnsurer(
    @Qualifier("responseProducerProperties") private val responseProducerProperties: ProducerProperties,
            ...
            component.orgIds.forEach { orgId ->
                coreKafkaAdmin.createOrModifyTopics(
                    KafkaTopics.eventTopic(
                        eventTopicNames.event("${component.domainName}-${component.packageName}-response", orgId),
                        partitions,
                        defaultReplicas,
                        responseProducerProperties.retentionTime
                    )
                )
            }
        }
